...
# collect `EvalSetup` modules from `flexeval/scripts/flexeval_lm.py`
all_core_modules["EvalSetup"] = [
    module
    for module in _get_all_modules("flexeval.scripts.flexeval_lm")
    if module.startswith("flexeval.scripts.flexeval_lm")
]

for name, module_classes in sorted(all_core_modules.items()):
    # sort classes so that base classes come first
    module_classes.sort(key=lambda x: ("base" not in x, x))
